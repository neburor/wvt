...
{  
	...
	public static function Stream($a,$b) {
		...
		$time = strtotime($arreglo[4]); 
		$date = date("d/m/y", $time);
			if($arreglo[2]=="contact"){
				echo '<p class="text-left"><i class="fa fa-user icon"></i> <span class="comment-date pull-right">'.$date.'</span><br>'.$arreglo[5].'</p>';
			}
			if($arreglo[2]=="reply"){
	...
	public static function Form($a,$b,$c) {
		...
		if($form_type=='chat'){
		echo '<div class="form-group"><div class="input-group"><input type="text" name="message" placeholder="Su mensaje a la administración ..." class="form-control" maxlength="1024" minlength="5" required></input><div class="input-group-btn"><button type="submit" class="btn btn-default"><i class="fa fa-share"></i></button></div></div></div>';
		}
		if ($opc_email=='show'){ echo '<label class="col-xs-12"><input type="checkbox" name="options"><span>Notificarme a mi correo.</span></label>';
